        ...
        for (let i=0; i<mass_data.length;i++){
            ...
            "<div class='row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative'>"+
              "<div class='col p-4 d-flex flex-column position-static'>"+
                ...
                // "</div>"+
                "<button class='btn btn-secondary'id='kcarzin' onclick='detete_tovar_korzina("+tovar['id']+");'>Удалить</button>"+
                
              "</div>"+
              "<div class='col-auto d-none d-lg-block'>"+
              "<img class='bd-placeholder-img' width='200' height='250' src='imge/products/"+tovar['photo']+"' alt='Generic placeholder image' >"+
            //     "<svg class='bd-placeholder-img' width='200' height='250' xmlns='http://www.w3.org/2000/svg' role='img' "+
            //    "aria-label='Placeholder: Thumbnail'"+
                 "<title>Placeholder</title><rect width='100%' height='100%' fill='#55595c'></rect><text x='50%' y='50%' fill='#eceeef' dy='.3em'></text>"+
               "</svg>"+
               "</div></div></div>";
...
function detete_tovar_korzina(id){
 ...
 add_all_orders.onclick=function(){
    ...
    message_zacaz.style.display="none";
